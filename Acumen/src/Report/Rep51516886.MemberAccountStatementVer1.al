...
Report 51516886 "Member Account Statement(Ver1)"
{
    ...
    dataset
    {
        dataitem("Member Register"; "Member Register")
        {
            ...
            dataitem(Benfund; "Cust. Ledger Entry")
            {
                DataItemLink = "Customer No." = field("No."), "Posting Date" = field("Date Filter");
                ...
                column(PostingDate_Benfund; Benfund."Posting Date")
                {
                }
                column(DocumentNo_Benfund; Benfund."Document No.")
                {
                }
                column(Description_Benfund; Benfund.Description)
                {
                }
                column(Amount_Benfund; Benfund.Amount)
                {
                }
                column(UserID_Benfund; Benfund."User ID")
                {
                }
                column(DebitAmount_Benfund; Benfund."Debit Amount")
                {
                }
                column(CreditAmount_Benfund; Benfund."Credit Amount")
                {
                }
                column(TransactionType_Benfund; Benfund."Transaction Type")
                {
                }
                column(OpenBalanceBenfund; OpenBalanceBenfund)
                {
                }
                column(ClosingBalanceBenfund; ClosingBalanceBenfund)
                {
                }
                column(BenfundBF; BenfundBF)
                {
                }
                trigger OnPreDataItem();
                begin
                    ClosingBalanceBenfund := BenfundBF;
                    OpenBalanceBenfund := BenfundBF;
                end;

                trigger OnAfterGetRecord();
                begin
                    ClosingBalanceBenfund := ClosingBalanceBenfund + (Benfund.Amount * -1);
                end;
