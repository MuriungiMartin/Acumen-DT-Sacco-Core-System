...
dotnet // --> Reports ForNAV Autogenerated code - do not delete or modify
{
    ...
    {
        type(ForNav.Report_6_3_0_2259; ForNavReport51516244_v6_3_0_2259) { }
    }
} // Reports ForNAV Autogenerated code - do not delete or modify -->

Report 51516244 "Loan Appraisal"
{
    RDLCLayout = './Layouts/LoanAppraisal.rdlc';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            ...
            column(AmountBoosted_LoansRegister; "Loans Register"."Boosted Amount")
            {
            }
            column(CommissiononBoosting_LoansRegister; LoanApp."Boosting Commision")
            {
            }
            ...
            dataitem("Loan Offset Details"; "Loan Offset Details")
            {
                ...
                column(CMInterestTopUp_LoanOffsetDetails; "Loan Offset Details"."Interest Top Up")
                {
                }
                column(CMAdditionalTopUpCommission_LoanOffsetDetails; "Loan Offset Details"."Additional Top Up Commission")
                {
                }
                column(CMTotalTopUp_LoanOffsetDetails; "Loan Offset Details"."Total Top Up")
                {
                }
            ...
            dataitem(LOANOFFSET; "Loan Offset Details")
            {
                DataItemLink = "Loan No." = field("Loan  No.");
                PrintOnlyIfDetail = false;
                column(ReportForNavId_31; 31) { } // Autogenerated by ForNav - Do not delete
                column(LoanNo_LOANOFFSET; LOANOFFSET."Loan No.")
                {
                }
                column(LoanTopUp_LOANOFFSET; LOANOFFSET."Loan Top Up")
                {
                }
                column(ClientCode_LOANOFFSET; LOANOFFSET."Client Code")
                {
                }
                column(LoanType_LOANOFFSET; LOANOFFSET."Loan Type")
                {
                }
                column(PrincipleTopUp_LOANOFFSET; LOANOFFSET."Principle Top Up")
                {
                }
                column(InterestTopUp_LOANOFFSET; LOANOFFSET."Interest Top Up")
                {
                }
                column(TotalTopUp_LOANOFFSET; LOANOFFSET."Total Top Up")
                {
                }
                trigger OnPreDataItem();
                begin
            ...
            begin
                ...
                if Cust.Get("Loans Register"."Client Code") then begin
                        ...
                        if LoanApp.Find('-') then begin
                            repeat
                                ...
                                if LoanApp."Outstanding Balance" > 0 then begin
                                    ...
                                    //
                                    //MESSAGE('%1 LoanApp.Repayment',LoanApp.Repayment);
                                    //END;
                                end;
                        ...
                        repeat
                            ...
                            if LoanTopUp.Find('-') then begin
                                repeat
                                    ...
                                    TopCommision := TopCommision + LoanTopUp.Commision;
                            ...
                            if LoanApp.Find('-') then begin
                                repeat
                                    ...
                                    LoanType.SetRange(LoanType."Appraise Deposits", true);
                    ...
                    */
                    ...
                    Psalary := TwoThirds - (Band + StatDeductions + OTHERDEDUCTIONS);   //changed
                    ...
                    if Collateral.Find('-') then begin
                        repeat
                            ...
                            Message(Format(CollCharge));
                        until Collateral.Next = 0;
                        ///Netdisbursed:=CollCharge;
                        Modify;
                    end;
                    //Recommended Amount
                    if "Loan Product Type" <> '' then begin
                        DepX := DEpMultiplier + CollCharge + GShares;
                    ...
                    PREMATURE := 0;
                    /*//**********added************
                    LegalFee:="Loans Register"."legal fee";
                    ValuationFee:="Loans Register".valuation;
                    ...
                    if LoanType.Get("Loans Register"."Loan Product Type") then begin
                        ...
                        LoanInsurance := 0;
                        // LoanInsurance:=SFactory.FnGetChargeFee("Loans Register"."Loan Product Type","Loans Register"."Approved Amount",'INS');
                        InsuranceFee := ROUND((((5.03 * "Loans Register".Installments + 3.03) * "Loans Register"."Approved Amount" / 6000) * 0.5), 0.05, '>');
                        //"Loan Insurance":=LoanInsurance;
                        ProccessingFee := SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", '');
                        LoanFormFee := SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'LPF');
                        Message(Format(LoanFormFee));
                        PREMATURE := SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'LAPF');
                        EmergencyUpfront := SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'INT');
                        ADVFEE := SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'ADVFEE');
                        // FACI
                        ...
                        Upfronts := LoanInsurance + EmergencyUpfront + ProccessingFee + LoanFormFee + ADVFEE + LAppraisalFee + LoanTransferFee + BridgeLevy + "Loans Register"."Boosted Amount" +
                        ...
                        "Loans Register".Modify;
                        Netdisbursed := "Loans Register"."Approved Amount" - Upfronts - "Loans Register"."Top Up Amount";
                    end;
                    //"Loans Register"."Capitalized Charges":=LAppraisalFee+AccruedInt+SaccoInt;
                    ...
                    "Loans Register".Modify;
                    //////////END CHARGE
                    /*IF "Loans Register"."Discount Loan"=TRUE THEN
                      BEGIN
                        DiscountFee:=(GenSetUp."Loan Discounting Commission(%)"*"Loans Register"."Approved Amount")/100;
                        END;*/
                    //***********************End Charges*********************************//
                    //Update Amount for Deboosting*************************
                    Booster := 0;
                    if LoanAp.Get("Loans Register"."Loan  No.") then begin
                        MaxDepositQualification := DEpMultiplier - (LOANBALANCE - BRIGEDAMOUNT);
                        //IF (MaxDepositQualification<"Requested Amount") AND ("Debooster Loan Applied"=TRUE) THEN BEGIN
                        //IF ("Debooster Loan Applied"=TRUE) THEN BEGIN
                        //MESSAGE('%1',"Loans Register"."Loan  No.");
                        if LoanType.Get("Loan Product Type") then begin
                            // DeboosterAmount:=ROUND(("Requested Amount"-MaxDepositQualification)/LoanType."Deposits Multiplier",1,'>');
                            // "Amount Boosted":=DeboosterAmount;
                            // MESSAGE('StatDeductions is %1',"Amount Boosted");
                            // "Commission on Boosting":=ROUND(DeboosterAmount*5/100);//(LoanType."Comission on Deboosting(%)"/100),1,'>');
                            // Booster:="Commission on Boosting";
                            Message('StatDeductions is %1', Booster);
                            LoanAp.Modify;
                            //END;
                        end;
                    end;
                    ...
                    //uncommented repayments
                    ///KIONGOZI-Repayments for Amortised method
                    ...
                    if LoanType.Get("Loan Product Type") then begin
                        ...
                        if "Mode of Disbursement" = "mode of disbursement"::Cheque then
                            Upfronts := BRIGEDAMOUNT + ProccessingFee + DisbursementFee + InsuranceFee + TopUpFee + DiscountFee + "Boosted Amount" + "Boosting Commision" + TotalFee
                        else
                ...
                Netdisbursed := ROUND("Approved Amount" - Upfronts, 0.05, '>');//+LoanInsurance+LoanProcessingFee+LoanAppraisal+CollCharge+TopUpComm),1,'<');
                                                                               //Netdisbursed:=ROUND(Netdisbursed,1,'<');
                                                                               //Netdisbursed:=ROUND(LoanApp."Approved Amount"-Upfronts,1,'<');
                                                                               //MESSAGE('%1',Upfronts);
                "Loans Register"."Amount Disbursed" := Netdisbursed;
    ...
    var
        ...
        Comment______Caption_Control1102755070Lbl: label 'Comment :____________________________________________________________________________________';
    ...
    var
        ObjLoanCharge: Record "HR Interview Specific Evaluatn";
    begin
        /*ObjLoanCharge.RESET;
		ObjLoanCharge.SETRANGE(ObjLoanCharge.,ProductCode);
    ...
    var
        [WithEvents]
        ReportForNav: DotNet ForNavReport51516244_v6_3_0_2259;
