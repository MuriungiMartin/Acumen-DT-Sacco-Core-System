...
Report 51516401 "Process Salaries Batch"
{
	RDLCLayout = './Layouts/ProcessSalariesBatch.rdlc'; DefaultLayout = RDLC;
	...
	begin
		...
		if ReceiptAllocations."Transaction Type" = ReceiptAllocations."transaction type"::Commission then begin
		if Abs(GenJournalLine.Amount) = 100 then
		InsCont:=100;
		GenJournalLine.Amount:=-25;
		end;
		GenJournalLine.Validate(GenJournalLine.Amount);
		if ReceiptAllocations."Transaction Type" = ReceiptAllocations."transaction type"::Commission then
		...
		if (ReceiptAllocations."Transaction Type" = ReceiptAllocations."transaction type"::Commission)
