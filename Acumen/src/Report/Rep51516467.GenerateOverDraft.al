...
{
	RDLCLayout = './Layouts/GenerateOverDraft.rdlc'; DefaultLayout = RDLC;

	dataset
	{
		dataitem(Vendor;Vendor)
		{
			DataItemTableView = sorting("No.") where("Creditor Type"=const("FOSA Account"),"Account Type"=filter('SAVINGS'|'SAVINGSREL'),"Balance (LCY)"=filter(<1.000),"Account Category"=filter(<>Branch),Status=filter(<>Closed|Deceased),Blocked=const(" "),"Account Category"=filter(<>Branch));
