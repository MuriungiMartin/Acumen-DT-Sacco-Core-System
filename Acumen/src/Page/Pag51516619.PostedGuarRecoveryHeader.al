...
Page 51516619 "Posted Guar Recovery Header"
{
    ...
    begin
        if LoansRec.Get("Loan to Attach") then begin
            ...
            GenJournalLine."account type"::Employee, LoansRec."Client Code", "Loan Disbursement Date", TDefaulterLoan * -1, Format(LoanApps.Source), EXTERNAL_DOC_NO,
            ...
            if LoanApp.Find('-') then begin
                repeat
                    if RunningBalance > 0 then begin
                        ...
                        GenJournalLine."account type"::Employee, LoanApp."Client Code", "Loan Disbursement Date", AmountToDeduct * -1, Format(LoanApp.Source), EXTERNAL_DOC_NO,
                ...
                GenJournalLine."account type"::Employee, "Member No", "Loan Disbursement Date", "Total Interest Due Recovered", 'BOSA', EXTERNAL_DOC_NO,
    ...
    begin
        begin
            if LoansRec.Get("Loan to Attach") then begin
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Loan Repayment",
                GenJournalLine."account type"::Employee, LoansRec."Client Code", "Loan Disbursement Date", "Deposits Aportioned" * -1, Format(LoansRec.Source), EXTERNAL_DOC_NO,
                Format(GenJournalLine."transaction type"::"Loan Repayment"), "Loan to Attach");
                //--------------------RECOVER-----------------------------
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                GenJournalLine."account type"::Employee, "Member No", "Loan Disbursement Date", "Deposits Aportioned", Format(LoansRec.Source), EXTERNAL_DOC_NO,
    ...
    begin
        if RunningBalance > 0 then begin
            ...
            if LoanApp.Find('-') then begin
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Loan Repayment",
                GenJournalLine."account type"::Employee, LoanApp."Client Code", "Loan Disbursement Date", "Mobile Loan" * -1, 'FOSA', EXTERNAL_DOC_NO,
                Format(GenJournalLine."transaction type"::"Loan Repayment"), LoanApp."Loan  No.");
                //--------------------RECOVER-----------------------------
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                GenJournalLine."account type"::Employee, "Member No", "Loan Disbursement Date", "Mobile Loan", 'BOSA', EXTERNAL_DOC_NO,
    ...
    begin
        if RunningBalance > 0 then begin
            ...
            if LoanApp.Find('-') then begin
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                GenJournalLine."account type"::Employee, "Member No", "Loan Disbursement Date", "Total Thirdparty Loans", 'BOSA', EXTERNAL_DOC_NO,
                Format(GenJournalLine."transaction type"::"Deposit Contribution") + '-' + LoanApp."Loan Product Type", '');
            end;
            exit(RunningBalance);
        end;
    ...
    begin
        ...
        if LoanDetails.FindSet then begin
            repeat
                ...
                GenJournalLine."account type"::Employee, LoanDetails."Guarantor Number", "Loan Disbursement Date", LoanDetails."Defaulter Loan", Format(LoansRec.Source::BOSA), "Loan to Attach",
        ...
        if LoansRec.Get("Loan to Attach") then begin
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Loan Repayment",
            GenJournalLine."account type"::Employee, LoansRec."Client Code", "Loan Disbursement Date", DLoanAmount * -1, Format(LoanApps.Source), EXTERNAL_DOC_NO,
