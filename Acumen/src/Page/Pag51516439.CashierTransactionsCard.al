...
Page 51516439 "Cashier Transactions Card"
{
        ...
        area(processing)
        {
            action(Post)
            {
                ...
                begin
                    ...
                    //******************************************************************8888
                    if ("Savings Product" = '300') or ("Savings Product" = '400') or ("Savings Product" = '500') or
                      ("Savings Product" = '600') or ("Savings Product" = '700') then begin

                        if Authorised = Authorised::No then begin
                            "Authorisation Requirement" := 'Withdrawal Above teller Limit';
                            Modify;
                            Error('You cannot withdraw From this Account seek Authorisation');
                            exit;
                    ...
                    if "Notifly Sms" = true then begin
                        NotiflySms();
                    end;
    ...
    begin
        ...
        //*****************************Charge Overdraft Comission*******************************//
        //*******************************************************************************RTGS**************
        // // IF "Transaction Mode New" = "Transaction Mode New"::RTGS THEN BEGIN
        // //
        // //        LineNo:=LineNo+10000;
        // //
        // //        GenJournalLine.INIT;
    ...
    begin
            ...
            if Account."Staff Account" <> true then begin
                ...
                if TransactionCharges.Find('-') then begin
                    repeat
                        ...
                        GenJournalLine."Line No." := LineNo;
                        GenJournalLine."Account Type" := GenJournalLine."account type"::"G/L Account";
                        GenJournalLine."Account No." := '60545';//TransactionCharges."G/L Account";
                        GenJournalLine.Validate(GenJournalLine."Account No.");
                        GenJournalLine."Posting Date" := "Transaction Date";
                        GenJournalLine.Description := 'Excise' + TransactionCharges.Description;
                        GenJournalLine.Validate(GenJournalLine."Currency Code");
        ...
        if "Transaction Mode New" = "transaction mode new"::RTGS then begin
            ...
            if TransactionCharges.Find('-') then begin
                ...
                GenJournalLine."Line No." := LineNo;
                GenJournalLine."Account Type" := GenJournalLine."account type"::"G/L Account";
                GenJournalLine."Account No." := '10305';//TransactionCharges."G/L Account";
                GenJournalLine.Validate(GenJournalLine."Account No.");
                GenJournalLine."Posting Date" := "Transaction Date";
                GenJournalLine.Description := 'Withdrawal Fees';//TransactionCharges.Description;
                GenJournalLine.Validate(GenJournalLine."Currency Code");
                GenJournalLine.Amount := TransactionCharges."RTGS Amount" * -1;
                GenJournalLine.Validate(GenJournalLine.Amount);
                GenJournalLine.Validate(GenJournalLine."Bal. Account No.");
                GenJournalLine."Shortcut Dimension 1 Code" := DActivity;
                GenJournalLine."Shortcut Dimension 2 Code" := DBranch;
                ...
                GenJournalLine.Description := 'Withdrawal Fees';//TransactionCharges.Description;
                GenJournalLine.Validate(GenJournalLine."Currency Code");
                GenJournalLine.Amount := TransactionCharges."RTGS Amount";
                GenJournalLine.Validate(GenJournalLine.Amount);
                //GenJournalLine."Bal. Account Type":=GenJournalLine."Bal. Account Type"::"Bank Account";
                //GenJournalLine."Bal. Account No.":=ChBank;
                //GenJournalLine.VALIDATE(GenJournalLine."Bal. Account No.");
    ...
    begin
        ...
        if (Type = 'Withdrawal') and ("Use Graduated Charges" = true) then begin
            ...
            if (Type = 'Withdrawal') and ("Use Graduated Charges" = false) then begin
                ...
                TransactionCharges.SetRange(TransactionCharges."Transaction Type", "Transaction Type");
                if TransactionCharges.Find('-') then begin
                    if (Amount <= 50000) then begin
                        ChargeAmount := TransactionCharges."charges Amount 1-50000";
                    end else
                        if (Amount >= 50001) and (Amount <= 100000) then begin
                            ChargeAmount := TransactionCharges."charges Amount 50001-100000";
                        end else
                            if (Amount >= 100001) and (Amount <= 500000) then begin
                                ChargeAmount := TransactionCharges."charges Amount 100001-500000";
                            end else
                                if (Amount >= 500001) and (Amount <= 1000000) then begin
                                    ChargeAmount := TransactionCharges."charges Amount 500001-1000000";
                                end else
                                    if (Amount >= 1000001) then begin
                                        ChargeAmount := TransactionCharges."charges Amount >1000001";
                                    end;
                    // ChargeAmount:=TransactionCharges."Charge Amount";
    ...
    begin
        ...
        SMSMessage.Source := 'NOTIFY';
        SMSMessage."Entered By" := UserId;
        SMSMessage."Sent To Server" := SMSMessage."sent to server"::No;
        SMSMessage."SMS Message" := 'Dear Member,your FOSA Account has' + "Transaction Type" + ''
        ...
        VendorApp.Reset;
        VendorApp.SetRange(VendorApp."No.", No);
        if VendorApp.Find('-') then begin
            // MESSAGE ();
            ...
            ChargeAmount := 0;
            Charges.Reset;
            Charges.SetRange(Charges.Code, '3');
            if Charges.Find('-') then begin
            ...
            GenJournalLine."Document No." := VendorApp."No.";
            GenJournalLine."Line No." := LineNo;
            GenJournalLine."Account Type" := GenJournalLine."account type"::Vendor;
            GenJournalLine."Account No." := VendorApp."No.";
            GenJournalLine."External Document No." := VendorApp."ID No.";
            GenJournalLine.Validate(GenJournalLine."Account No.");
            GenJournalLine."Posting Date" := Today;
            GenJournalLine.Description := 'Notify charge';
            GenJournalLine.Validate(GenJournalLine."Currency Code");
            GenJournalLine.Amount := ChargeAmount;
            GenJournalLine.Validate(GenJournalLine.Amount);
            GenJournalLine."Bal. Account Type" := GenJournalLine."bal. account type"::"G/L Account";
            ...
            GenJournalLine.Validate(GenJournalLine."Account No.");
            GenJournalLine."Posting Date" := Today;
            GenJournalLine.Description := 'Excise ' + 'Notify Charge';
            GenJournalLine.Validate(GenJournalLine."Currency Code");
            GenJournalLine.Amount := (ChargeAmount * 0.2);
            GenJournalLine.Validate(GenJournalLine.Amount);
            GenJournalLine."Bal. Account Type" := GenJournalLine."bal. account type"::"G/L Account";
            GenJournalLine."Bal. Account No." := GenSetup."Excise Duty Account";
            GenJournalLine.Validate(GenJournalLine."Bal. Account No.");
