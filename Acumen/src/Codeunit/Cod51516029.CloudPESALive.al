...
Codeunit 51516029 "CloudPESALive"
{

    trigger OnRun()
    begin

        //MESSAGE(RegisteredMemberDetails('254728648534'));
        //MESSAGE(OutstandingLoansUSSD('254720401314'));
        //MESSAGE(GetBalanceEnquiry('BAL95455','254710985344',TODAY));
        ...
        //FnsentSMS();
        //MESSAGE('%1',AdvanceEligibility('01-002-1797'));
        //SendSchedulesms();
        //MESSAGE(LoanRepayment('01-002-0943','LN2840','000201679966049',10000));
    ...
    procedure GetBalanceEnquiry(referenceNumber: Code[20]; phoneNumber: Text[13]; transactionDate: Date) response: Text[1024]
    var
    ...
    begin
        ...
        if CloudPESATrans.Find('-') then begin
            ...
            chargesTable.Reset;
            chargesTable.SetRange(chargesTable.Code, GenLedgerSetup."Mobile Charge");
            if chargesTable.Find('-') then begin
                chargesTable.TestField(chargesTable."GL Account");
                MobileCharges := chargesTable."Charge Amount";
            ...
            SaccoGenSetup.Get;
            exciseDutyAmount := (SaccoGenSetup."Excise Duty(%)" / 100) * (MobileCharges + CloudPESACharge);

            TotalCharges := exciseDutyAmount + (MobileCharges + CloudPESACharge);
            ...
            if vendorTable.Find('-') then begin
                ...
                if (TempBalance >= TotalCharges) then begin
                    ...
                    accBalance := TempBalance - TotalCharges;//Vendor."Balance (LCY)"-(Vendor."ATM Transactions"+Vendor."Uncleared Cheques"+Vendor."EFT Transactions"+miniBalance);
                    ...
                    if vendorTableOther.Find('-') then begin
                        repeat
                            vendorTableOther.CalcFields("Balance (LCY)");
                            AccountTypes.Reset;
                            AccountTypes.SetRange(AccountTypes.Code, vendorTableOther."Account Type");
                            if AccountTypes.Find('-') then begin
                                response += '0|OK|' + AccountTypes.Description + '|' + Format(vendorTableOther."Balance (LCY)") + ';';
                            end;
                    ...
                    CloudPESATrans.Charge := MobileCharges + CloudPESACharge;// ** TotalCharges; //** exclude excise duty which will be recomputed when posting
                    ...
                    CloudPESATrans.Status := CloudPESATrans.Status::Pending;
                    CloudPESATrans.Comments := 'Pending';
    ...
    procedure GetMinistatement(referenceNumber: Code[50]; phoneNumber: Code[50]; transactionDate: Date; noOfEntries: Integer) response: Text
    var
        vendorTable: Record Vendor;
    ...
    begin
        ...
        if CloudPESATrans.Find('-') then begin
            ...
            vendorTable.Reset;
            vendorTable.SetRange(vendorTable."Mobile Phone No", phoneNumber);//Vendor.SETRANGE(Vendor."BOSA Account No",FnGetMemberNo(Phone));
            if vendorTable.Find('-') then begin
                ...
                if (TempBalance >= TotalCharges) then begin
                    ...
                    if vendorLedgerEntries.FindSet then begin
                        ...
                        repeat
                            ...
                            if minimunCount >= noOfEntries then exit
    ...
    procedure WithdrawCommit(documentNo: Text[20]; traceid: Text[30]; telephoneNo: Text[20]; account: Text[20]; amount: Decimal; charges: Decimal; transactionDate: Date; appType: Code[100]; description: Text[100]) response: Text[1024]
    var
        BufferTable: Record "ATM Transactions";
    ...
    begin
        ...
        if cloudpesaTransTable.Find('-') then begin
            ...
            cloudpesaTransTable."Transaction Type" := cloudpesaTransTable."transaction type"::Withdrawal;//TransType;
            ...
            if Members.Find('-') then begin
                ...
                if LoansTable.Find('-') then begin
                    ...
                    if AppType = 'USSD' then begin
                        SMSMessage(Ref, Members."No.", phone, CopyStr(msg, 1, 250), '');
                    end;
            ...
            if amount <> 2 then begin
                if amount <> 3 then begin
                    ...
                    if Members."Shares Retained" < 5000 then begin
                        amount := 4;
                        Res := Format(amount) + '::::' + 'You do not qualify for eloan because your share capital is below requireded amount of Ksh. 5,000 ::::false';
                        exit;
...
*/
        ...
        begin
            ...
            if Members.Find('-') then begin
                ...
                if Vendor.Find('-') then begin
                    ...
                    ExcDuty := (20 / 100) * (MobileCharges);
                    TotalCharges := CloudPESACharge + MobileCharges + ExcDuty + MPESACharge;
                    Bal := Bal - TotalCharges - TempBalance;
                end;
            end;
        end;
        ...
        if CloudPESATrans.Find('-') then begin
            ...
            if Members.Find('-') then begin
                ...
                /* IF AppType='1' THEN BEGIN
                     Members.CALCFIELDS(Members."Watoto Savings");
                     TempBalance:=Members."Watoto Savings";
                  END;
                  IF AppType='2' THEN BEGIN
                     Members.CALCFIELDS(Members."Member withdrawable Deposits");
                     TempBalance:=Members."Member withdrawable Deposits";
                  END;
                ...
                if (TempBalance > amount) then begin
                    ...
                    GenJournalLine."External Document No." := Members."No.";
                    GenJournalLine."Posting Date" := Today;
                    GenJournalLine.Description := 'MPESA Withdrawal-' + Members."No." + '' + Members.Name;
                    GenJournalLine.Amount := (amount) * -1;
                    GenJournalLine.Validate(GenJournalLine.Amount);
                    if GenJournalLine.Amount <> 0 then
                        GenJournalLine.Insert;
    ...
    begin
        ...
        MobileChargesACC := GenLedgerSetup."Sacco Charge Account";
